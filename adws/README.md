...
adws/
   ...
   adw_chore_implement.py            # Compound workflow (plan → implement)
   adw_e2e_test.py                   # E2E testing with Playwright MCP
   adw_triggers/
...
### 5. E2E Testing Workflow: `adw_e2e_test.py`

Execute end-to-end tests using Playwright MCP for browser automation.
...
- Optional test spec file generation
- Comprehensive test reporting

**Workflow Phases:**
1. **Initialize Browser**: Navigate to target URL and capture initial state
2. **Execute Test Steps**: Perform user interactions and validations
3. **Capture Evidence**: Screenshots, snapshots, console logs, network requests
4. **Generate Report**: Comprehensive test results with pass/fail status
5. **Create Spec** (optional): Generate persistent `.spec.ts` file for future runs
...
Best for: User journey testing, regression testing, accessibility validation, visual testing
...
1. **Use the Right Tool**:
   ...
   - `adw_e2e_test.py` for end-to-end testing with Playwright MCP
   - `adw_sdk_prompt.py` for type-safe SDK operations or interactive sessions

2. **Model Selection**:
   - Use `sonnet` (default) for most tasks
   - Use `opus` for complex reasoning, large codebases, or comprehensive E2E tests
